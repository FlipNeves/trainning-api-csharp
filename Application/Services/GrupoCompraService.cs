...
{
    public class GrupoCompraService : IGrupoCompraService
    {
        private IGrupoCompraRepository _repository;

        public GrupoCompraService(IGrupoCompraRepository grupoCompraRepository)
        {
            _repository = grupoCompraRepository;
        }

        private static GrupoCompraDTO MapToDTO(GrupoCompra x)
            => new GrupoCompraDTO { Codigo = x.Codigo, Descricao = x.Descricao };
        ...
        public async Task<RespostaDTO<GrupoCompraDTO>> NovoRegistroAsync(string descricao)
        {
            if (string.IsNullOrWhiteSpace(descricao))
                return RespostaDTO.BadRequest<GrupoCompraDTO>("Descrição é obrigatória.");
